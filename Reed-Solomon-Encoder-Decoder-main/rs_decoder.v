module rs_decoder (
    ...
    // --- THÊM BIẾN ĐẾM FIXED LOGIC CHO VALID OUT ---
    reg [8:0] out_cnt; // Bộ đếm quản lý chính xác chu kỳ xuất 255 byte đầu ra
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            // =================================================================
            // FIXED LOGIC CHO VALID_OUT: Chỉ bật cờ khi có dữ liệu sạch chui ra
            // =================================================================
            if (bm_done) begin
                out_cnt   <= 1;    
                valid_out <= 1'b0; 
                ...
                // CHỈ PHẤT CỜ CHO 223 BYTE DỮ LIỆU GỐC!
                if (out_cnt <= 223) begin
                    valid_out <= 1'b1;
                end else begin
                    valid_out <= 1'b0; // 32 byte Parity trôi qua -> Hạ cờ xuống, không cho nhận!
                end
                
                // Vẫn phải đếm đủ 255 nhịp để reset hệ thống cho gói tin tiếp theo
                if (out_cnt == 255) begin
                    out_cnt <= 0;  
                end else begin
                    out_cnt <= out_cnt + 1; 
                end
